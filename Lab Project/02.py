...
class AdminUser(User):
    ...
    def search_doctor(self, name):
        result = [doc.display() for doc in self.__doctors.values() if name.lower() in doc.get_name().lower()]
        return "\n".join(result) if result else "No doctor found with that name."

    def search_patient(self, name):
        result = [pat.display() for pat in self.__patients.values() if name.lower() in pat.get_name().lower()]
        return "\n".join(result) if result else "No patient found with that name."

    def discharge_patient(self, patient_id):
        if patient_id not in self.__patients:
            raise KeyError("Patient ID not found.")
        patient = self.__patients[patient_id]
        total_bill = patient.get_bill()
        print(f"Total Bill Amount: ${total_bill}")

        print("Choose Payment Method: 1. Cash 2. Credit")
    ...
    def display_panel(self):
        while True:
            ...
            if admin_choice == "1":
                ...
                print(self.search_doctor(name))

            elif admin_choice == "6":
                name = input("Enter patient name to search: ")
                print(self.search_patient(name))
