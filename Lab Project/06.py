...
class AdminUser(User):
    ...
    def search_doctor(self, search_term):
        ...
        for doctor in self.__doctors.values():
            if str(doctor.get_person_id()) == search_term or search_term.lower() in doctor.get_specialization().lower():
                results.append(doctor.display())
        return "\n".join(results) if results else "No matching doctors found."

    def search_patient(self, search_term):
        results = []
        for patient in self.__patients.values():
            if str(patient.get_person_id()) == search_term or any(search_term.lower() in symptom.lower() for symptom in patient.get_symptoms()):
                results.append(patient.display())
        return "\n".join(results) if results else "No matching patients found."
    ...
    def discharge_patient(self, patient_id):
        ...
        print(f"Discharging Patient: {patient.get_name()}")
        print(f"Outstanding Bill: ${patient.get_bill()}")

        print("Select Payment Method:")
        print("1. Cash")
        print("2. Credit")
        choice = input("Choose payment method (1/2): ")

        if choice == "1":
            amount_paid = float(input("Enter cash amount: "))
            if amount_paid >= patient.get_bill():
                change = amount_paid - patient.get_bill()
        ...
        print(f"Patient {patient.get_name()} discharged successfully.")
    ...
    def schedule_appointment(self):
        ...
        try:
            ...
            patient = self.__patients[patient_id]
            print(doctor.schedule_appointment(patient, appointment_time, bill_amount))
        except KeyError as e:
            print(f"Error: {str(e)}. Make sure both Doctor and Patient IDs are correct.")
    ...
    def display_panel(self):
        while True:
            ...
            if admin_choice == "1":
                ...
                pat_id = int(input("Enter Patient ID to discharge: "))
                self.discharge_patient(pat_id)
